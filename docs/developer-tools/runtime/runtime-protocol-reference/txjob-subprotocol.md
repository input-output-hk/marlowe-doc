...
| 3. `Withdraw` | none | `WithdrawError` | `TxBody BabbageEra` | | Builds a transaction that withdraws funds paid to a role by a contract. |
|  | | | | `v` | The version of the contract. |
...
| 4. `Submit` | `SubmitStatus` | `WithdrawError` | `TxBody BabbageEra` | | Submits a transaction to the upstream node and waits for confirmation. |
